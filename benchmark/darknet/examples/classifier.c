...
void launch() {
    char* line = strtok(workload, "\n");
    int top = atoi(line);

    char* datacfg = strtok(NULL, "\n");
    char* cfgfile = strtok(NULL, "\n");
    char* weightfile = strtok(NULL, "\n");
	...
	if(filename == NULL) {
		outputResult(buff, 256);
		break;
	}
#endif
    }
	duration += sec(clock()-time);
    printf("Entire duration: %f\n", duration);
}
