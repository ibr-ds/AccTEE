...
#if defined(TIMESTAMP) || defined(WASM)
#include <sys/time.h>
#endif
...
static char *ulltostr(uint64_t value, char *ptr, int base) {
  uint64_t t = 0, res = 0;
  uint64_t tmp = value;
  int count = 0;
...
// This is the javascript function to be called with the result.
// "offset" is a pointer to the string with the result and "length" its length
extern "C" {
    extern void outputResult(char* offset, int length);

    // This function is called from javascript directly after the module is instantiated.
    // Unless required its implementation should not be changed.
...
// This is the "main" function of a module. It is called by javascript after
// "allocateWorkload" is used to allocate space for the workload and it has been
// copied to the returned address.
// Once this function finishes, its result should be returned via the "outputResult"
// function.

void launch() {

    // We assume a workload in the form of MMNN\0
    // Allocate strings to parse M and N

    char mString[3];
	mString[0] = workload[0];
	mString[1] = workload[1];
	mString[2] = '\0';
	char nString[3];
	nString[0] = workload[2];
	nString[1] = workload[3];
	nString[2] = '\0';

	uint32_t max_set_value = parse_t<uint32_t>(mString);
	uint32_t subset_size = parse_t<uint32_t>(nString);
...
#if defined(TIMESTAMP) || defined(WASM)
	uint64_t duration = GetTimeStamp() - startTime;
    char* ret = (char*) malloc(20);
    for(int i=0; i < 20; i++) {ret[i] = 0;};
    cout << "It took me " << duration << " milliseconds." << endl;
...
#ifdef WASM
    ulltostr(duration, ret, 10);
    outputResult(ret, strlen(ret));
    //string str = strs.str();
    //outputResult((char*)str.c_str(), (int) strlen(str.c_str()));
#endif
